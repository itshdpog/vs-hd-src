	...
	*/
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				case 'smphd': //smphds stage
				var back:BGSprite = new BGSprite('smphdback', -170, -70, 0.9, 0.9);
				add(back);

				var smphdfront:BGSprite = new BGSprite('smphdfront', -200, 0, 0.9, 0.9);
				smphdfront.setGraphicSize(Std.int(smphdfront.width * 1.1));
				smphdfront.updateHitbox();
				add(smphdfront);
				...
				var smphdfront1:BGSprite = new BGSprite('smphdfront', -200, 0, 0.9, 0.9);
				smphdfront1.setGraphicSize(Std.int(smphdfront1.width * 1.1));
				smphdfront1.updateHitbox();
				add(smphdfront1);



				case 'oldbg': //oldbg stage
				var oldback:BGSprite = new BGSprite('oldback', -170, -200, 0.9, 0.9);
				add(oldback);
		...
		botplayTxt.setFormat(Paths.font("coolvetica.ttf"), 32, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
//		openChartEditor();
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (daNote.isParent && daNote.tooLate && !daNote.isSustainNote && ClientPrefs.inputSystem == 'Kade Engine')
				{
	...
	private function onKeyRelease(event:KeyboardEvent):Void
	{
		...
		if(!cpuControlled && !paused && key > -1)
		{
			...
			if (ClientPrefs.inputSystem == 'Kade Engine') keysPressedKE[key] = false;
